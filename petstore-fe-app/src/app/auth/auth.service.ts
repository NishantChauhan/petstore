...
export class AuthService {
  ...
  getUserStatus(): boolean {
    this.accessToken = localStorage.getItem('petapp-accesstoken');
    if (this.accessToken) {
  ...
  getUser(): Observable<User> {
    ...
    return this.http.get<User>(url).pipe(catchError(undefined));
  }
  getUserDetails(): User {
    const tempUser = new User();
    tempUser.username = localStorage.getItem('petapp-username');
    tempUser.username = localStorage.getItem('petapp-userrole');
    return tempUser;
  }
  ...
  obtainToken(user: User): Observable<ExecutionStatus> {
    ...
    const url =
      `${this.serverUrl}/oauth/token?username=${this.authUser.username}&password=${
    ...
    return this.http.post<any>(url, options).pipe(
      map(data => {
        ...
        localStorage.setItem('petapp-accesstoken', data.access_token);
 ...
 */
    ...
    localStorage.removeItem('petapp-accesstoken');
    localStorage.removeItem('petapp-username');
    localStorage.removeItem('petapp-userrole');

    return this.http
      .post<ExecutionStatus>(url, options)
      .pipe(catchError(this.handleError<ExecutionStatus>()));
  }
